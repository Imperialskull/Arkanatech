...
public class ArkaneC4 extends Block {
    private Icon blockIconTop, blockIconBottom;

    public ArkaneC4(int par1, Material par2Material) {
        super(par1, Material.tnt);
        this.setUnlocalizedName(Strings.C4_NAME);
    ...
    public void registerIcons(IconRegister iconRegister) {
        this.blockIcon = iconRegister
                .registerIcon(Reference.MOD_ID.toLowerCase()+ ":"+Strings.C4_SIDES);
        this.blockIconTop = iconRegister
                .registerIcon(Reference.MOD_ID.toLowerCase()+ ":"+Strings.C4_TOP);
        this.blockIconBottom = iconRegister
                .registerIcon(Reference.MOD_ID.toLowerCase()+ ":"+Strings.C4_BOTTOM);
    }
    ...
    public void onBlockDestroyedByExplosion(World par1World, int par2, int par3, int par4, Explosion par5Explosion)
    {
        ...
        {
            EntityC4Primed entityc4primed = new EntityC4Primed(par1World, (double)((float)par2 + 0.5F), (double)((float)par3 + 0.5F), (double)((float)par4 + 0.5F));
            entityc4primed.fuse = par1World.rand.nextInt(entityc4primed.fuse / 4) + entityc4primed.fuse / 8;
            par1World.spawnEntityInWorld(entityc4primed);
        }
    }
    ...
    public void func_94391_a(World par1World, int par2, int par3, int par4, int par5, EntityLivingBase par6EntityLivingBase)
    {
        if (!par1World.isRemote)
        {
            if ((par5 & 1) == 1)
            {
                EntityC4Primed entityc4primed = new EntityC4Primed(par1World, (double)((float)par2 + 0.5F), (double)((float)par3 + 0.5F), (double)((float)par4 + 0.5F));
                par1World.spawnEntityInWorld(entityc4primed);
                par1World.playSoundAtEntity(entityc4primed, "random.fuse", 1.0F, 1.0F);
            }
        }
    }
